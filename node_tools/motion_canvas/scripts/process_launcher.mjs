...
export const buildNpmServeLaunchSpec = ({platform = process.platform, comSpec, runtimeRoot, port}) => {
  ...
  const npmArgs = ['run', 'serve', '--', '--config', configPath, '--port', String(port), '--strictPort'];
  if (platform === 'win32') {
    ...
    const commandLine = `npm.cmd run serve -- --config ${quoteForCmd(configPath)} --port ${Number(port)} --strictPort`;
