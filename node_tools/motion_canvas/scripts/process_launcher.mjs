...
export const buildNpmServeLaunchSpec = ({platform = process.platform, comSpec, runtimeRoot, root, port}) => {
  ...
  const configPath = pathApi.join(runtimeRoot, 'vite.config.ts');
  const viteBin = pathApi.join(root, 'node_modules', '.bin', 'vite');
  const viteArgs = ['--host', '127.0.0.1', '--config', configPath, '--port', String(port), '--strictPort'];
  if (platform === 'win32') {
    // vite.cmd is a batch file. Node cannot execute it directly with shell:false;
